...
export default function AdvancedForm({ recordings }: { recordings: ZoomRecording[]}) {
  ...
  const form = useForm({
    ...
    validate: {
      audioFile: (value) => (source === 'upload' && value ? null : 'File is required'),
      title: (value) => (value ? null : 'Title is required'),
      date: (value) => (source === 'upload' && value ? null : 'Date is required'),
    },
  });
  ...
  return (
    <form
      onSubmit={(event) => {
        ...
        if(source === 'zoom-cloud') return handleSubmit(form.values);
      ...
      <Stack gap={40}>
        ...
        <Paper shadow="md" withBorder p={20}>
          ...
          {appState === APP_STATES.COMPLETED && (
            <Button fullWidth color="gray" onClick={() => setAppState(APP_STATES.INIT)}>Completed! Start new submission?</Button>
          )}
        </Paper>
      </Stack>
    </form>
